...
- `LiteECS` 的普通组件和 Flag 组件按添加顺序遍历；移除组件不会改变其他组件的遍历顺序，便于快照和帧同步保持稳定输出。
- `ViewECS` 的普通组件和 Flag 组件使用连续数组与尾部交换删除，优先保证遍历性能，不保证删除后的遍历顺序稳定。
- `LiteECS` 和 `ViewECS` 实体会记录自己实际持有的组件 ID，销毁实体时只清理这些组件，而不是扫描全部 Collector。
